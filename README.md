...
## 📂 Проекты
  🔧  Hexlet CV (волонтёрский вклад)
      Роль: Frontend-разработчик
      Стек:
      React, TypeScript, Inertia.js, Mantine UI, Zustand, i18next, MSW (Mock Service Worker), ESLint (FSD), Git
      Задача:
      Разработать страницу «Избранное» для личного кабинета пользователя в рамках волонтёрского проекта. Проект построен на архитектуре Feature-Sliced Design с бэкендом на Laravel и связкой через Inertia.js.
      Результат:
      Разработана полноценная страница с таблицей, пагинацией, адаптивным дизайном и скелетным загрузчиком.
      Настроена локализация (русский/английский) через i18next.
      Написаны мок-обработчики (MSW) для эмуляции API, что позволило вести разработку независимо от бэкенда.
      Спроектирована типизация данных (FavoriteDTO) с соблюдением правил FSD (публичное API через index.ts).
      Код прошёл полноценное код-ревью: исправлены замечания по TSDoc, семантике таблиц, стилям и архитектуре.
      Оценка и признание:
      Код признан референсным (эталонным) в команде и рекомендован как пример для других разработчиков (скелетный загрузчик, i18n, пагинация).
      Получено приглашение продолжить участие в проекте в рамках новой дорожной карты на основе качества выполненной работы.
