...
## 📂 Проекты
   🔧 Hexlet CV
  фронтенд-разработчик (волонтер)

  Стек: React, TypeScript, Inertia.js, Mantine UI, Zustand, i18next, MSW, ESLint (FSD), Git

  Задача: Разработать страницу «Избранное» для личного кабинета пользователя в рамках волонтёрского проекта. Проект построен на Feature-Sliced Design и Inertia.js с бэкендом на Laravel.

  Что сделано:
  Спроектирована и реализована полноценная страница с таблицей, пагинацией, адаптивным дизайном и Empty State.
  Настроена локализация (русский/английский) через i18next.
  Написаны мок-обработчики (MSW) для эмуляции API, что позволило разрабатывать независимо от бэкенда.
  Спроектирована типизация данных (FavoriteDTO) с соблюдением правил FSD (публичное API через index.ts).
  Код прошёл код-ревью: исправлены замечания по TSDoc, семантике, стилям и архитектуре.
  Ведение ветки, разрешение merge-конфликтов, поддержание чистой истории коммитов.

  Статус: Компонент разработан и готов к интеграции. Проект завершён по решению команды до вливания.
